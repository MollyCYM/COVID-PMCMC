...
model dureau {
  ...
  sub parameter {
    ...
    mu ~ truncated_gaussian(0.001, 0.00002, lower = 0)
    tau ~ uniform(0, 1)
  }
...
write.csv(fitS,"MS.csv")
fitS <- read.csv("MS.csv", header=TRUE, stringsAsFactors=FALSE)
g1 <- ggplot(data = fitS) +
...
write.csv(bi_lst$sigma$value,"Msigma.csv")
write.csv(bi_lst$gamma$value,"Mgamma.csv")
write.csv(bi_lst$beta$value,"Mbeta.csv")
write.csv(bi_lst$mu$value,"Mmu.csv")

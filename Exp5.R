...
library(rbi.helpers)
# Load the data
v <- read.csv("covidbm_daily.csv", header=FALSE, stringsAsFactors=FALSE) 
...
bi <- sample(bi_model, end_time = end_time, input = input_lst, obs = obs_lst, nsamples = 1000, nparticles = minParticles, nthreads = ncores, proposal = 'prior',seed=0123111) %>% 
...
write.csv(bi_lst,"../data/Exp5_model1.csv")
fitY <- bi_lst$y %>%
  group_by(time) %>%
  mutate(
...
write.csv(fitY,"../data/Exp5_y1.csv")
...
write.csv(plot_df,"../data/Exp5_beta1.csv")
...
write.csv(plot_df1,"../data/Exp5_beta01.csv")

Mmodel <- read.csv("Covidbm1.csv", header=TRUE, stringsAsFactors=FALSE)
S<-Mmodel[,4]
E<-Mmodel[,5]
I<-Mmodel[,6]
R<-Mmodel[,7]
...
write.csv(fitS,"../data/Exp5_S1.csv")
...
write.csv(fitE,"../data/Exp5_E1.csv")
...
write.csv(fitI,"../data/Exp5_I1.csv")
...
write.csv(fitR,"../data/Exp5_R1.csv")


write.csv(1/bi_lst$k$value,"../data/Exp5_alpha1.csv")
write.csv(1/bi_lst$gamma$value,"../data/Exp5_gamma1.csv")

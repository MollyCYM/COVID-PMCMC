...
bi <- sample(bi_model, end_time = end_time, input = input_lst, obs = obs_lst, nsamples = 1000, nparticles = minParticles, nthreads = ncores, proposal = 'prior') %>% 
  ...
  sample(nsamples = 500000, thin = 5)

bi_lst <- bi_read(bi %>% sample_obs)

write.csv(bi_lst, "../data/longdays.csv")
fitY <- bi_lst$y %>%
  group_by(time) %>%
  mutate(
...
write.csv(fitY,"../data/longdays_y.csv")
...
write.csv(fitS,"../data/longdays_S.csv")
...
write.csv(fitE,"../data/longdays_E.csv")
...
write.csv(fitI,"../data/longdays_I.csv")
...
write.csv(fitR,"../data/longdays_R.csv")
...
write.csv(fitM,"../data/longdays_M.csv")

write.csv(bi_lst$sigma$value,"../data/longdays_sigma.csv")
write.csv(bi_lst$gamma$value,"../data/longdays_gamma.csv")
write.csv(bi_lst$beta$value,"../data/longdays_beta.csv")
write.csv(bi_lst$mu$value,"../data/longdays_mu.csv")

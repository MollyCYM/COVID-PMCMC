...
dim i(size = 259)
  ...
  sub transition(delta = h) {
    ...
    ode(alg = 'RK4(3)', h = 1.0, atoler = 1.0e-3, rtoler = 1.0e-8) {
      dx[i]/dt = theta*(a+b*Forcing[i]-x[i])+sigma*e[i]/h
      dS[i]/dt = -exp(x[i])*S[i]*I[i]/N
      dE[i]/dt = exp(x[i])*S[i]*I[i]/N - E[i]/k
      dI[i]/dt = E[i]/k-I[i]/gamma
      dR[i]/dt = I[i]/gamma
      dZ[i]/dt = E[i]/k
    }
  }
  ...
  sub observation {
    y[i] ~ log_normal(log(max(Z[i]/5.0, 0)), tau)
  }
...
model <- bi_model(lines = stringi::stri_split_lines(model_str)[[1]])
bi_model <- libbi(model)
input_lst <- list(N = 52196381, Forcing[i]=Forcing[i])
end_time <- max(y$time)
obs_lst <- list(y = y %>% dplyr::filter(time <= end_time))

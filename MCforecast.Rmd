...
require(deSolve)
require(truncnorm)
fitY <- read.csv("forecast1_y1.csv", header=TRUE, stringsAsFactors=FALSE)
fitbeta <- read.csv("forecast1_beta1.csv", header=TRUE, stringsAsFactors=FALSE)
fitk <- read.csv("forecast1_alpha1.csv", header=TRUE, stringsAsFactors=FALSE)
fitgamma <- read.csv("forecast1_gamma1.csv", header=TRUE, stringsAsFactors=FALSE)
fitsigma <- read.csv("forecast1_sigma1.csv", header=TRUE, stringsAsFactors=FALSE)
fittheta <- read.csv("forecast1_theta1.csv", header=TRUE, stringsAsFactors=FALSE)
fittau <- read.csv("forecast1_tau1.csv", header=TRUE, stringsAsFactors=FALSE)
fita <- read.csv("forecast1_a1.csv", header=TRUE, stringsAsFactors=FALSE) 
fitb <- read.csv("forecast1_b1.csv", header=TRUE, stringsAsFactors=FALSE) 
...
gen_ode_model <- function(rep_time = 1000, ori_seed = 20241, tims = 85, 
                          N = 52196381,ini_state = c(t=1,S=3358042.5, E=1523287, I=9078503, R=38236548)
                          ){
  ...
  taus = setNames(Zs, paste0("tau", 1:rep_time))
  ...
  for (rep_i in 1:rep_time) {
    ...
    theta = fittheta[rep_i,2] ## 0.1
    sigma = fitsigma[rep_i,2]
    tau = fittau[rep_i,2]
    x = ornstein_uhlenbeck(tims, theta, sigma, x0=0, tims,mu,e)
    
    k = fitk[rep_i,2] ##
    gamma = fitgamma[rep_i,2]##
    params = c(k, gamma)
...
ode_example <- gen_ode_model(rep_time = 1000)
...
write.csv(ode_example,"forcastMC.csv")

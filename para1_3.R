...
bi <- sample(bi_model, end_time = end_time, input = input_lst, init=init_list, obs = obs_lst, nsamples = 1000, nparticles = minParticles, nthreads = ncores, filter= 'lookahead', proposal = 'model',seed=0123112) %>% 
...
write.csv(bi_lst,"../data/para1_model3.csv")
fitY <- bi_lst$y %>%
  group_by(time) %>%
  mutate(
...
write.csv(fitY,"../data/para1_y3.csv")
...
write.csv(plot_df,"../data/para1_beta3.csv")

Mmodel <- read.csv("Covidbm12.csv", header=TRUE, stringsAsFactors=FALSE)
S<-Mmodel[,4]
E<-Mmodel[,5]
I<-Mmodel[,6]
R<-Mmodel[,7]
...
write.csv(fitS,"../data/para1_S3.csv")
...
write.csv(fitE,"../data/para1_E3.csv")
...
write.csv(fitI,"../data/para1_I3.csv")
...
write.csv(fitR,"../data/para1_R3.csv")

write.csv(1/bi_lst$k$value,"../data/para1_alpha3.csv")

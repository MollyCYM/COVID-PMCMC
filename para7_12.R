...
library(rbi.helpers)
# Load the data
v <- read.csv("covidoudg3_y12w.csv", header=FALSE, stringsAsFactors=FALSE) %>%
  rowSums()
...
model dureau {
  ...
  sub parameter {
    ...
    c ~ truncated_gaussian(-0.3, 0.1, upper = 0)
  }
...
init_list <- list(k=5, gamma=9, sigma=sqrt(0.004),theta=0.05,tau=0.1,a=-0.02,b=-0.2,c=-0.3)
...
write.csv(bi_lst,"../data/para7_model12.csv")
fitY <- bi_lst$y %>%
  group_by(time) %>%
  mutate(
...
write.csv(fitY,"../data/para7_y12.csv")
...
write.csv(plot_df,"../data/para7_beta12.csv")

Mmodel <- read.csv("covidoudg3_model12.csv", header=TRUE, stringsAsFactors=FALSE)
S<-Mmodel[-1,7]
E<-Mmodel[-1,9]
I<-Mmodel[-1,11]
R<-Mmodel[-1,13]
...
write.csv(fitS,"../data/para7_S12.csv")
...
write.csv(fitE,"../data/para7_E12.csv")
...
write.csv(fitI,"../data/para7_I12.csv")
...
write.csv(fitR,"../data/para7_R12.csv")


write.csv(bi_lst$k$value,"../data/para7_alpha12.csv")
write.csv(bi_lst$gamma$value,"../data/para7_gamma12.csv")
write.csv(bi_lst$sigma$value,"../data/para7_sigma12.csv")
write.csv(bi_lst$tau$value,"../data/para7_tau12.csv")
write.csv(bi_lst$theta$value,"../data/para7_theta12.csv")
write.csv(bi_lst$a$value,"../data/para7_a12.csv")
write.csv(bi_lst$b$value,"../data/para7_b12.csv")
write.csv(bi_lst$c$value,"../data/para7_c12.csv")

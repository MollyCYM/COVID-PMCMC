...
model dureau {
  ...
  sub parameter {
    k ~ truncated_gaussian(0.5, 1, lower = 0) // k is the period here, not the rate, i.e. 1/k is the rate
    gamma ~ truncated_gaussian(12, 1, lower = 0) // gamma is the period, not the rate
    sigma ~ truncated_gaussian(0.2, 0.1, lower = 0)
    theta ~ truncated_gaussian(0.1, 0.01, lower = 0)
    tau ~ truncated_gaussian(0.2, 0.01, lower = 0)
    a ~ truncated_gaussian(-0.2, 0.01, upper = 0)
    b ~ truncated_gaussian(-0.3, 0.1, upper = 0)
  }
...
write.csv(fitY,"../data/para55_y83.csv")

plot_df <- bi_lst$x  %>%
  group_by(time) 
write.csv(plot_df,"../data/para55_x83.csv")
...
write.csv(fitS,"../data/para55_S83.csv")
...
write.csv(fitE,"../data/para55_E83.csv")
...
write.csv(fitI,"../data/para55_I83.csv")
...
write.csv(fitR,"../data/para55_R83.csv")

write.csv(bi_lst$loglikelihood$value,"../data/para55_loglik83.csv")
write.csv(bi_lst$k$value,"../data/para55_alpha83.csv")
write.csv(bi_lst$gamma$value,"../data/para55_gamma83.csv")
write.csv(bi_lst$sigma$value,"../data/para55_sigma83.csv")
write.csv(bi_lst$tau$value,"../data/para55_tau83.csv")
write.csv(bi_lst$theta$value,"../data/para55_theta83.csv")
write.csv(bi_lst$a$value,"../data/para55_a83.csv")
write.csv(bi_lst$b$value,"../data/para55_b83.csv")

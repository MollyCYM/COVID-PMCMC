...
model dureau {
  ...
  Z <- ((t_now) % 7 == 0 ? 0 : Z)
    ...
    ode(alg = 'RK4(3)', h = 1.0, atoler = 1.0e-3, rtoler = 1.0e-8) {
      ...
      dE/dt = exp(x)*S*(0.1*I+E)/N - E*(1/5+1/5)
      dI/dt = E/5-I*(1/5+0.0087)
...
bi <- sample(bi_model, end_time = end_time, input = input_lst, obs = obs_lst, nsamples = 1000, nparticles = minParticles, nthreads = ncores, proposal = 'model',seed=111123) %>% 
...
write.csv(bi_lst,"../data/para2_model1.csv")
fitY <- bi_lst$y %>%
  group_by(time) %>%
  mutate(
...
write.csv(fitY,"../data/para2_y1.csv")
...
write.csv(plot_df,"../data/para2_beta1.csv")
...
write.csv(fitS,"../data/para2_S1.csv")
...
write.csv(fitE,"../data/para2_E1.csv")
...
write.csv(fitI,"../data/para2_I1.csv")
...
write.csv(fitR,"../data/para2_R1.csv")

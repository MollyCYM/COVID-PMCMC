...
library(rbi.helpers)
# Load the data
v <- read.csv("covidoudg2_y921w.csv", header=FALSE, stringsAsFactors=FALSE) %>%
  rowSums()
...
model dureau {
  ...
  sub parameter {
    tau ~ truncated_gaussian(0.05, 0.1, lower = 0)
  }
...
write.csv(bi_lst,"../data/para5_model1131434.csv")
fitY <- bi_lst$y %>%
  group_by(time) %>%
  mutate(
...
write.csv(fitY,"../data/para5_y1131434.csv")
...
write.csv(plot_df,"../data/para5_beta1131434.csv")

Mmodel <- read.csv("covidoudg2_model921.csv", header=TRUE, stringsAsFactors=FALSE)
S<-Mmodel[-1,7]
E<-Mmodel[-1,9]
I<-Mmodel[-1,11]
R<-Mmodel[-1,13]
...
write.csv(fitS,"../data/para5_S1131434.csv")
...
write.csv(fitE,"../data/para5_E1131434.csv")
...
write.csv(fitI,"../data/para5_I1131434.csv")
...
write.csv(fitR,"../data/para5_R1131434.csv")



write.csv(bi_lst$tau$value,"../data/para5_tau1131434.csv")

...
library(rbi.helpers)
# Load the data
v <- read.csv("rsimcovidou_daily1.csv", header=FALSE, stringsAsFactors=FALSE) 
...
model dureau {
  ...
  sub parameter {
    ...
    gamma ~ truncated_gaussian(9, 0.1, lower = 0) // gamma is the period, not the rate
...
init_list <- list(k=2, gamma=9, sigma=sqrt(0.004),theta=0.05,tau=0.1,a=-0.02,b=-0.2)

bi <- sample(bi_model, end_time = end_time, input = input_lst, init=init_list, obs = obs_lst, nsamples = 2000, nparticles = minParticles, nthreads = ncores, proposal = 'model',seed=006666) %>% 
...
write.csv(bi_lst,"../data/para2_model5.csv")
fitY <- bi_lst$y %>%
  group_by(time) %>%
  mutate(
...
write.csv(fitY,"../data/para2_y5.csv")
...
write.csv(plot_df,"../data/para2_beta5.csv")

Mmodel <- read.csv("rsimcovidou_model1.csv", header=TRUE, stringsAsFactors=FALSE)
S<-Mmodel[,4]
E<-Mmodel[,5]
I<-Mmodel[,6]
R<-Mmodel[,7]
...
write.csv(fitS,"../data/para2_S5.csv")
...
write.csv(fitE,"../data/para2_E5.csv")
...
write.csv(fitI,"../data/para2_I5.csv")
...
write.csv(fitR,"../data/para2_R5.csv")


write.csv(1/bi_lst$k$value,"../data/para2_alpha5.csv")
write.csv(1/bi_lst$gamma$value,"../data/para2_gamma5.csv")
write.csv(bi_lst$sigma$value,"../data/para2_sigma5.csv")
write.csv(bi_lst$tau$value,"../data/para2_tau5.csv")
write.csv(bi_lst$theta$value,"../data/para2_theta5.csv")
write.csv(bi_lst$a$value,"../data/para2_a5.csv")
write.csv(bi_lst$b$value,"../data/para2_b5.csv")

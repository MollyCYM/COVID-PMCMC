...
bi <- sample(bi_model, end_time = end_time, input = input_lst, init=init_list, obs = obs_lst, nsamples = 1000, nparticles = minParticles, nthreads = ncores, proposal = 'prior',seed=0123112) %>% 
  adapt_particles(min = minParticles, max = minParticles*500) %>%
  sample(nsamples = 10000, thin = 1)

bi_lst <- bi_read(bi %>% sample_obs)

write.csv(bi_lst,"../data/para1_model11.csv")
fitY <- bi_lst$y %>%
  group_by(time) %>%
  mutate(
...
write.csv(fitY,"../data/para1_y11.csv")
...
write.csv(plot_df,"../data/para1_beta11.csv")
...
write.csv(fitS,"../data/para1_S11.csv")
...
write.csv(fitE,"../data/para1_E11.csv")
...
write.csv(fitI,"../data/para1_I11.csv")
...
write.csv(fitR,"../data/para1_R11.csv")

write.csv(1/bi_lst$k$value,"../data/para1_alpha11.csv")
write.csv(1/bi_lst$gamma$value,"../data/para1_gamma11.csv")
write.csv(bi_lst$sigma$value,"../data/para1_sigma11.csv")
write.csv(bi_lst$tau$value,"../data/para1_tau11.csv")

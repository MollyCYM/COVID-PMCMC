...
model StochasticVolatility {
  ...
  sub parameter {
    ...
    phi ~ truncated_gaussian(0.9, 0.1, lower = -1, upper=1)
    sigma_eta ~ truncated_gaussian(sqrt(0.04), 0.1, lower=0)
  }
  
  sub proposal_parameter {
    beta ~ truncated_gaussian(beta, 0.2, lower = 0) 
    phi ~ truncated_gaussian(phi, 0.1, lower = -1, upper=1) 
    sigma_eta ~ truncated_gaussian(sigma_eta, 0.005)
  }
...
write.csv(bi_lst,"../data/svminf_model22.csv")
fitY <- bi_lst$y %>%
  group_by(time) %>%
  mutate(
...
write.csv(fitY,"../data/svminf_y22.csv")
...
write.csv(fitalpha,"../data/svminf_alpha22.csv")

write.csv(bi_lst$beta$value,"../data/svminf_beta22.csv")
write.csv(bi_lst$phi$value,"../data/svminf_phi22.csv")
write.csv(bi_lst$sigma_eta$value,"../data/svminf_sigma22.csv")
...
library(rbi.helpers)
# Load the data
v <- read.csv("seairbm_y_day1.csv", header=FALSE, stringsAsFactors=FALSE) 
...
model dureau {
  ...
  sub parameter {
    ...
    tau ~ truncated_gaussian(0.1, 0.02, lower = 0)
  }
  
  sub proposal_parameter {
    k ~ truncated_gaussian(k, 0.0001, lower = 0) 
    gamma ~ truncated_gaussian(gamma, 0.0001, lower = 0) 
    sigma ~ truncated_gaussian(sigma, 0.000001, lower = 0)
    tau ~ gaussian(tau, 0.000001)
  }
...
write.csv(fitY,"../data/para55_y12.csv")

plot_df <- bi_lst$x  %>%
  group_by(time) 
write.csv(plot_df,"../data/para55_x12.csv")

Mmodel <- read.csv("seairbm_model1.csv", header=TRUE, stringsAsFactors=FALSE)
S<-Mmodel[-1,7]
E<-Mmodel[-1,9]
I<-Mmodel[-1,11]
R<-Mmodel[-1,13]
...
write.csv(fitS,"../data/para55_S12.csv")
...
write.csv(fitE,"../data/para55_E12.csv")
...
write.csv(fitI,"../data/para55_I12.csv")
...
write.csv(fitR,"../data/para55_R12.csv")

write.csv(bi_lst$loglikelihood$value,"../data/para55_loglik12.csv")
write.csv(bi_lst$k$value,"../data/para55_alpha12.csv")
write.csv(bi_lst$gamma$value,"../data/para55_gamma12.csv")
write.csv(bi_lst$sigma$value,"../data/para55_sigma12.csv")
write.csv(bi_lst$tau$value,"../data/para55_tau12.csv")

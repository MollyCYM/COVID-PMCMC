...
gen_ode_model <- function(rep_time = 200, ori_seed = 2023, tims = 365, 
  ...
  for (rep_i in 1:rep_time) {
    ...
    x = ornstein_uhlenbeck(tims, 0.05, sqrt(0.004), log(0.8), tims,mu,e)
...
for (i in 1:zs) {
  if (i == 1){
    plot(ode_example$Z[,1], col="blue", type = "l", 
         ylim = c(0, max(ode_example$Z)),
         xlab = 'Times', ylab = 'Z')
    }
  else{
    lines(ode_example$Z[,i], col="blue")
  }
}
...
for (i in 1:ys) {
  if (i == 1){
    plot(ode_example$Y[,1], col="red", type = "l", 
         ylim = c(0, max(ode_example$Y)),
         xlab = 'Times', ylab = 'Y')
  }
  else{
    lines(ode_example$Y[,i], col="red")
  }
}
...
for (i in 1:es) {
  if (i == 1){
    plot(ode_example$dE[,1], col="yellow", type = "l", 
...
for (i in 1:is) {
  if (i == 1){
    plot(ode_example$dI[,1], col="pink", type = "l", 
...
for (i in 1:rs) {
  if (i == 1){
    plot(ode_example$dR[,1], col="green", type = "l", 

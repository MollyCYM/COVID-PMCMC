...
bi <- sample(bi_model, end_time = end_time, input = input_lst, init=init_list,target="posterior", obs = obs_lst, nsamples = 2000, nparticles = minParticles, nthreads = ncores, proposal = 'model',seed=123) %>% 
  adapt_particles(min = minParticles, max = minParticles*100) %>%
  adapt_proposal(min = 0.05, max = 0.4) %>%
  sample(nsamples =10, thin = 1)

bi_lst <- bi_read(bi %>% sample_obs)

write.csv(bi_lst,"para3_model252.csv")
fitY <- bi_lst$y %>%
  group_by(time) %>%
  mutate(
...
write.csv(plot_df,"para3_beta252.csv")
...
write.csv(fitE,"para3_E252.csv")



write.csv(1/bi_lst$k$value,"para3_alpha252.csv")
write.csv(bi_lst$tau$value,"para3_tau252.csv")
